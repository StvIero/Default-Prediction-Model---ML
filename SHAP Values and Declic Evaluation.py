# -*- coding: utf-8 -*-
"""
Created on Fri May 17 09:47:48 2024
...
shap_values_summary = shap_values_df.abs().mean().sort_values(ascending=False)
print("\nSHAP Values Summary:\n", shap_values_summary)
...
def declic_eval(y_true, y_pred_proba, num_sets=10):
    ...
    declic_result = df_y.groupby('set')['true'].mean() * 100
    print("\nDeclic Evaluation:\n", declic_result)
    
    fig_width, fig_height = 6.4 * 1.5, 4.8
    fig = plt.figure(figsize=(fig_width, fig_height))
    ax = declic_result.plot(kind='bar')
    ax.axhline(y=y_true.mean() * 100, color='red', linestyle='--')
    
    for p in ax.patches:
        ax.annotate(f'{p.get_height():.2f}%', xy=(p.get_x() + p.get_width() / 2., p.get_height()), ha='center', va='bottom', xytext=(0, 0), textcoords='offset points')
    ax.annotate(f'Mean Default Rate: {y_true.mean() * 100:.3f}%', xy=(fig_width * .5, y_true.mean() * 100), xytext=(0, 2), ha='left', va='bottom', color='red', textcoords='offset points')
    ...
    plt.show()
    return declic_result

# Declic Evaluation Output
declic_result = declic_eval(y_val, y_pred_val_proba)
